...
class FIRE(object):
    ...
    def train(self, cur_adj_mat, time_mat, cur_time, user_sim_mat=None, item_sim_mat=None):
        ...
        u, s, vt = svds(self.norm_adj, k=self.pri_factor, return_singular_vectors=True)
        self.vt = vt[::-1, :]
    ...
    def test(self):
        ...
        results = []
        if self.use_user_si:
            results.append(pool.apply_async(self.get_P1, (batch_test,)))
        if self.use_item_si:
            results.append(pool.apply_async(self.get_P2, (batch_test,)))
        results.append(pool.apply_async(self.get_P3, (batch_test,)))
        pool.close()
        pool.join()
